    ...
    <div class="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <form @submit.prevent="addNewBook">
        ...
        <div class="relative max-w-md  w-full min-w-[200px] mb-5">
          ...
          <textarea rows="20" v-model="book.description"
                    class="w-full bg-transparent placeholder:text-slate-400 text-slate-700 text-sm border border-slate-200 rounded-md px-3 py-2 transition duration-300 ease focus:outline-none focus:border-slate-400 hover:border-slate-300 shadow-sm focus:shadow"
                    placeholder="Enter book description"></textarea>
        </div>
        <div class="mb-5">
          <label for="formImageFile" class="block mb-2 text-sm text-slate-600">Book Cover Image</label>
          <input ref="bookCoverRef" type="file" id="formImageFile" accept="image/jpeg" @change="uploadBookCoverImage" class="bg-transparent placeholder:text-slate-400 text-slate-700 text-sm border border-slate-200 rounded-md px-3 py-2 transition duration-300 ease focus:outline-none focus:border-slate-400 hover:border-slate-300 shadow-sm focus:shadow" required />
        </div>
        <div class="text-center pt-10">
          <button class="p-3 px-8 text-white bg-blue-600 rounded-lg hover:opacity-70 focus:outline-none me-4" @click="handleCancel">Cancel</button>
    ...
    allGenres() {
      if (this.book.genres.length === 1) {
        ...
        for (let i = 0; i < this.book.genres.length; i++) {
          result += this.book.genres[i].genre_name.toLowerCase()
