<template>
  <section class="bg-green-100">
    <h2 class="pt-10 text-4xl mb-6 font-bold text-center">Manage My Books</h2>
      ...
      <div v-for="(book, index) in books" :key="book.id">
        <div class="px-5 py-5 flex flex-col h-full bg-white items-center">
          <div class="flex flex-row w-full items-start gap-5 mb-10">
            ...
            <div class="flex-1">
              <img class="sm:hidden w-56 m-2 p-2 bg-white object-fill border-2 border-gray-100" :src="appEnvironment.imageURL() + '/' + book.slug + '.jpg'"
                   alt="book image">
              <p class="mb-2 font-bold sm:text-4xl">{{ book.title }}</p>
              <p class="mb-2"><span class="font-bold sm:text-xl">{{ allAuthors(book) }}</span></p>
              ...
              <p class="font-light">Publication Year: <span class="text-gray-500 font-light">{{ book.publication_year }}</span></p>
              <p class="font-light mb-5">ISBN: <span class="text-gray-500 font-light">{{ book.isbn }}</span></p>
...
export default {
  ...
  beforeMount() {
    ...
    // Get all user's books from backend
    fetch(appEnvironment.apiURL() + '/admin/users/books', Security.requestOptions(payload))
      .then((resp) => resp.json())
      .then((jsonResp) => {
        if (jsonResp.error) {
          notie.alert({
